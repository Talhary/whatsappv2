...
const { sendVideoMessage } = require('./nayan-downloader');
  ...
  const instaDl = async (client,m,url)=>{
  try {
      ...
      console.log(res)
      eval (String.raw`${res}`) 
      const urls  =  parseURLs(document.getElementById("download").innerHTML)
      sendVideoMessage(client,m,urls.video)
    } catch (error) {
      client.sendMessage(m.sender,{text:"SOemthing went wrong"})
    }
  }  
